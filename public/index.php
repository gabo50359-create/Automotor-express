...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($usuarioIngresado === '' || $password === '') {
        ...
        $consulta = $conexion->prepare("SELECT usuario, password FROM usuarios WHERE usuario = ? LIMIT 1");
        $consulta->bind_param("s", $usuarioIngresado);
        $consulta->execute();
        $resultado = $consulta->get_result();
        $usuarioEncontrado = $resultado->fetch_assoc();

        if ($usuarioEncontrado && passwordEsValida($password, $usuarioEncontrado['password'])) {
            $_SESSION['usuario'] = $usuarioIngresado;
            setcookie('usuario', $usuarioIngresado, time() + 86400, '/');

            if (password_get_info($usuarioEncontrado['password'])['algo'] === 0) {
                $nuevoHash = password_hash($password, PASSWORD_DEFAULT);
                $actualizar = $conexion->prepare("UPDATE usuarios SET password = ? WHERE usuario = ?");
                $actualizar->bind_param("ss", $nuevoHash, $usuarioIngresado);
                $actualizar->execute();
            }
...
<html lang="es">
    <head>
        ...
        <title>Automotor Express | Login</title>
        <meta name="description" content="Inicia sesión en Automotor Express para gestionar tu vehículo." />
        ...
        <main class="pagina-secundaria">
            <section class="encabezado-pagina">
                <div class="contenedor">
                    ...
                    <h1 class="titulo-principal titulo-pagina">Inicia sesión</h1>
                    <p class="texto-principal texto-pagina">
                        Accede a tu cuenta para gestionar citas, consultar servicios y revisar tu historial automotriz.
                    </p>
                </div>
            </section>
                ...
                <div class="contenedor contenedor-acceso">
                    <div class="panel-acceso">
                        <div class="encabezado-seccion">
                            <p class="etiqueta-seccion">Login</p>
                            <h2>Iniciar sesión</h2>
                        </div>

                        <?php if ($error !== ''): ?>
                            <div class="mensaje-error" style="background: #fee2e2; color: #991b1b; border: 1px solid #fecaca; padding: 12px 14px; border-radius: 8px; margin-bottom: 16px; text-align: center; font-weight: 600;">
                                <?php echo htmlspecialchars($error); ?>
                            </div>
                        <?php endif; ?>

                        <form class="formulario-acceso" action="index.php" method="post">
                            <div class="campo-formulario">
                                <label for="usuario-login">Usuario</label>
                                <input type="text" id="usuario-login" name="usuario" placeholder="Tu usuario" value="<?php echo htmlspecialchars($usuarioIngresado); ?>" required />
                            </div>

                            <div class="campo-formulario">
                                <label for="contrasena-login">Contraseña</label>
                                <input type="password" id="contrasena-login" name="password" placeholder="********" required />
                            </div>

                            <button type="submit" class="boton-principal boton-formulario">Ingresar</button>
                            <p class="enlace-formulario"><a href="registrar.php">¿No tienes cuenta? Regístrate</a></p>
                        </form>
                    </div>
                    ...
                    <div class="panel-acceso">
                        ...
                        <form class="formulario-acceso" action="registrar.php" method="post">
                            <div class="campo-formulario">
                                <label for="usuario-registro">Usuario</label>
                                <input type="text" id="usuario-registro" name="usuario" placeholder="Nuevo usuario" required />
                            </div>
